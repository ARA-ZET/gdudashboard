...
export function DocList({ kind }: { kind: 'quote' | 'invoice' }) {
  ...
  return (
    <AdminShell title={`${label}s`} actions={<LinkButton href={`/admin/${kind}s/new`} variant="gold"><Icon name="plus" className="h-4 w-4" /> New {kind}</LinkButton>}>
      ...
      {!loaded ? (
        ...
        <EmptyState
          ...
          action={docs.length === 0 ? <LinkButton href={`/admin/${kind}s/new`} variant="gold"><Icon name="plus" className="h-4 w-4" /> New {kind}</LinkButton> : undefined}
        />
      ) : (
              ...
              <tbody className="divide-y divide-outline-variant/50">
                {filtered.map((d) => (
                  <tr key={d.id} className="cursor-pointer hover:bg-surface-dim/50">
                    <td className="px-4 py-3"><Link href={`/admin/${kind}s/${d.id}`} className="font-semibold text-navy hover:text-gold-700">{d.number}</Link></td>
