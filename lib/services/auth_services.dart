...
class AuthService {
  ...
  GduTeam? _userFromFirebaseUser(User? user) {
    if (user == null) {
      return null;
    }
    debugPrint(user.uid);

    return GduTeam(user.displayName, user.email, user.uid);
  }

  // Auth change user stream
  Stream<GduTeam?> get user {
    return _auth.authStateChanges().map(_userFromFirebaseUser);
  }

  Future<GduTeam?> signInAnonymously() async {
    try {
  ...
  Future<void> checkAndCreateUser(User user) async {
    ...
    if (!userSnapshot.exists) {
      createUser(FirebaseUser(user.displayName?.split(",")[0],
          user.displayName?.split(",")[1], user.email, user.uid));
    }
  }

  Future<GduTeam?> registerWithEmailAndPassword(BuildContext context,
    ...
    try {
      ...
      createUser(FirebaseUser(name, surname, email, result.user?.uid));
      final user = _userFromFirebaseUser(result.user);
