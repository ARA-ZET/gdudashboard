...
class _NewInvoiceState extends State<NewInvoice> {
  ...
  final GlobalKey<FormState> _formKeyClient = GlobalKey<FormState>();
  ...
  Widget build(BuildContext context) {
    ...
    InvNumber invNum = context.watch<InvoiceController>().inv;
    String invoiceNumber = invNum.invoice.toString().padLeft(6, "0");
                        ...
                        onPressed: () async {
                          if (_formKeyClient.currentState!.validate()) {
                            context.read<InvoiceController>()
                              ..updateInvNumber(invNum.invoice)
                              ..addInvoice(
                                context,
                                items,
                                Invoice(
                            ...
                            context.read<ClientsProvider>().updateClientBalance(
                                customer.uid, double.parse(total));
                            final data = await service.createInvoice(items,
                                customer, reference.text, invoiceNumber, date);
